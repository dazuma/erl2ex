...
defmodule Erl2ex.Codegen do
  ...
  defp write_form(context, header = %ExHeader{}, io) do
    ...
    if not Enum.empty?(header.records) do
      ...
      if header.record_info_available do
        context = context
          |> write_record_info_func(header.records, io)
      end
    end
    context
  end
  ...
  defp write_record_info_func(context, records, io) do
    context
      |> skip_lines(:attr, io)
      |> write_string("# The record_info function is auto-generated by the Erlang compiler.", io)
      |> foreach(records, io, fn(ctx, {name, fields}, io) ->
        ctx
