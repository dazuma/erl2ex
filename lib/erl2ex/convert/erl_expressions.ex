...
defmodule Erl2ex.Convert.ErlExpressions do
  ...
  def conv_expr({:fun, _, {:function, mod_expr, name_expr, arity_expr}}, context) do
    ...
    ex_expr =
      if is_atom(ex_name) and is_integer(ex_arity) do
  ...
  def conv_expr({:record_index, _, name, field}, context) do
    ...
    {{ModuleData.record_index_macro(context.module_data), [],
      [ModuleData.record_data_attr_name(context.module_data, name), ex_field]}, context}
  end
  ...
  defp conv_catch(expr, context) do
    catch_clauses = [
      ...
      {:->, [],
       [
         [:error, {:reason, [], Elixir}],
         {:EXIT, {{:reason, [], Elixir}, {{:., [], [:erlang, :get_stacktrace]}, [], []}}}
       ]}
    ]
  ...
  defp conv_type(:binary, [m, n], context) do
    ...
    {{:<<>>, [],
      [
        {:"::", [], [{:_, [], Elixir}, ex_m]},
        {:"::", [], [{:_, [], Elixir}, {:*, @import_kernel_metadata, [{:_, [], Elixir}, ex_n]}]}
      ]}, context}
  end
  ...
  defp record_field_list(record_name, fields, context) do
    ...
    {underscores, ex_fields} =
      Enum.split_with(ex_all_fields, fn
        {{:_, _, Elixir}, _} -> true
        {_, _} -> false
      end)
  ...
  defp update_map(base_map, assocs = [{:map_field_assoc, _, _, _} | _], context) do
    {inexact_assocs, remaining_assocs} =
      assocs
      |> Enum.split_while(fn
  ...
  defp conv_const(:FILE, _, context) do
    {{{:., [], [{:__aliases__, [alias: false], [:String]}, :to_char_list]}, [],
      [{{:., [], [{:__ENV__, [], Elixir}, :file]}, [], []}]}, context}
  end
  ...
  defp conv_call({:atom, _, :record_info}, [{:atom, _, :size}, {:atom, _, rec}], context) do
    {{ModuleData.record_size_macro(context.module_data), [],
      [ModuleData.record_data_attr_name(context.module_data, rec)]}, context}
  end
  ...
  defp conv_normal_call({:atom, _, func}, args, context) do
    ...
    ex_expr =
      case ModuleData.local_call_strategy(context.module_data, func, arity) do
        {:apply, mapped_name} ->
          {{:., [], [{:__aliases__, [alias: false], [:Kernel]}, :apply]}, [],
