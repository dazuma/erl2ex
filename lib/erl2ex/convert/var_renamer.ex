...
defmodule Erl2ex.Convert.VarRenamer do
  ...
  def compute_var_maps(expr, extras \\ []) do
    {var_names, func_names} = expr
      |> collect_variable_names({MapSet.new, MapSet.new})
    var_names = var_names |> MapSet.union(extras |> Enum.into(MapSet.new))
