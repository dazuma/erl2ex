...
defmodule Erl2ex.Convert do
  ...
  def module(erl_module, opts \\ []) do
    ...
    if Context.needs_record_info?(context) do
      forms = [build_record_info(context) | forms]
    end
  ...
  defp header_check_form(%Erl2ex.ExRecord{}, header), do:
    %Erl2ex.ExHeader{header | require_record: true}
  ...
  defp build_record_info(context) do
    records = Context.map_records(context, fn(name, fields) -> {name, fields} end)
    %Erl2ex.ExRecordInfo{
      comments: ["# The record_info function is auto-generated by the Erlang compiler."],
      records: records
    }
  end
