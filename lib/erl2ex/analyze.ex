...
defmodule Erl2ex.Analyze do
  ...
  def local_call_strategy(%AnalyzedModule{funcs: funcs, imports: imports}, name, arity) do
    ...
    if Map.has_key?(func_info.arities, arity) do
      ...
      cond do
        not is_legal_function_name(mapped_name) ->
          {:apply, mapped_name}
        MapSet.member?(@elixir_reserved_words, mapped_name)
            or MapSet.member?(@elixir_special_forms, mapped_name)
            or Map.has_key?(imports, mapped_name) ->
          {:qualify, mapped_name}
        true ->
          {:bare, mapped_name}
      end
  ...
  def func_renamer_name(%AnalyzedModule{func_renamer: func_renamer}) do
    func_renamer
  end
  ...
  def is_deffable_function_name(name) do
    is_legal_function_name(name) and not MapSet.member?(@elixir_reserved_words, name)
  end
  ...
  defp add_func_info({name, arity}, analysis) do
    ...
    used_attr_names = analysis.used_attr_names
    used_func_names = analysis.used_func_names
    func_renamer = analysis.func_renamer
    if func_info.func_name == {} do
      can_be_defd = is_deffable_function_name(name)
      bare_name_ok = can_be_defd and
      ...
      if func_info.is_exported or bare_name_ok do
        ...
        if func_renamer == nil and not can_be_defd do
