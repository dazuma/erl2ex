...
defmodule Erl2ex.Pipeline.Parse do
  ...
  defp preprocess_tokens_for_erl(form_tokens), do: preprocess_tokens_for_erl(form_tokens, [])

  defp preprocess_tokens_for_erl([], result), do: Enum.reverse(result)

  defp preprocess_tokens_for_erl([{:"?", _}, {:"?", _}, {:atom, line, name} | tail], result),
    do: preprocess_tokens_for_erl(tail, [{:var, line, :"??#{name}"} | result])

  defp preprocess_tokens_for_erl([{:"?", _}, {:"?", _}, {:var, line, name} | tail], result),
    do: preprocess_tokens_for_erl(tail, [{:var, line, :"??#{name}"} | result])

  defp preprocess_tokens_for_erl([{:"?", _}, {:atom, line, name} | tail], result),
    do: preprocess_tokens_for_erl(tail, [{:var, line, :"?#{name}"} | result])

  defp preprocess_tokens_for_erl([{:"?", _}, {:var, line, name} | tail], result),
    do: preprocess_tokens_for_erl(tail, [{:var, line, :"?#{name}"} | result])

  defp preprocess_tokens_for_erl([{:comment, _, _} | tail], result), do: preprocess_tokens_for_erl(tail, result)
  defp preprocess_tokens_for_erl([tok | tail], result), do: preprocess_tokens_for_erl(tail, [tok | result])
  ...
  defp parse_erl_define(line, macro_tokens, replacement_tokens, opts) do
    ...
    replacement_exprs =
      case :erl_parse.parse_exprs(replacement_tokens) do
        {:ok, asts} ->
          [asts]
  ...
  defp preprocess_tokens_for_ext([tok | tail], result), do: preprocess_tokens_for_ext(tail, [tok | result])
