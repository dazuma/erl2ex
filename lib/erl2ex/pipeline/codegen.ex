...
defmodule Erl2ex.Pipeline.Codegen do
  ...
  defp write_form(
         context,
         %ExRecord{tag: tag, macro: macro, data_attr: data_attr, fields: fields, comments: comments},
         io
       ) do
  ...
  defp write_raw_attr(context, name, register, arg, io) do
    context =
      if register do
        context
        |> write_string(
          "Module.register_attribute(__MODULE__, #{expr_to_string(name)}, persist: true, accumulate: true)",
          io
        )
      else
        context
