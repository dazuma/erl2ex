...
defmodule Erl2ex.Pipeline.Analyze do
  ...
  defp collect(accumulator, elements, fun), do: Enum.reduce(elements, accumulator, fun)
        ...
        :export ->
          ErlSyntax.on_trees1(arg_nodes, module_data, fn arg_node ->
            ErlSyntax.on_arity_qualifier_list(arg_node, module_data, fn mod_data, name, arity ->
              ...
              %ModuleData{mod_data | exports: exports}
            end)
          end)
        ...
        :export_type ->
          ErlSyntax.on_trees1(arg_nodes, module_data, fn arg_node ->
            ErlSyntax.on_type_with_arity_list(arg_node, module_data, fn mod_data, name, arity ->
              %ModuleData{mod_data | type_exports: MapSet.put(mod_data.type_exports, {name, arity})}
            end)
          end)
  ...
  defp handle_form_for_used_attr_names({_erl_ast, form_node}, module_data) do
    ErlSyntax.on_static_attribute(form_node, module_data, fn attr_name, _arg_nodes ->
      if Names.special_attr_name?(attr_name) do
        ...
        %ModuleData{module_data | used_attr_names: MapSet.put(module_data.used_attr_names, attr_name)}
      end
    end)
  end
  ...
  defp handle_form_for_funcs({_erl_ast, form_node}, module_data) do
    ErlSyntax.on_type(form_node, :function, module_data, fn ->
      ...
      ErlSyntax.on_atom(name_node, module_data, fn name ->
        module_data = %ModuleData{module_data | local_funcs: MapSet.put(module_data.local_funcs, {name, arity})}
        ...
        if not Map.has_key?(func_rename_map, name) and
             (MapSet.member?(module_data.exports, {name, arity}) or
                (Names.local_callable_function_name?(name) and
                   not Map.has_key?(module_data.imported_funcs, name))) do
          %ModuleData{
            module_data
  ...
  defp assign_local_func_names(module_data) do
    ...
    |> Enum.reduce(module_data, fn {name, _arity}, cur_data ->
      ...
      if Map.has_key?(func_rename_map, name) do
        ...
        %ModuleData{module_data | func_rename_map: func_rename_map, used_func_names: used_func_names}
      end
    end)
  end
  ...
  defp handle_form_for_records({{:attribute, _line, :record, {recname, fields}}, _form_node}, module_data) do
    macro_name = Utils.find_available_name(recname, module_data.used_func_names, "erlrecord")
    data_name = Utils.find_available_name(recname, module_data.used_attr_names, "erlrecordfields")
  ...
  defp handle_form_for_records({{:function, _line, _name, _arity, clauses}, _form_node}, module_data) do
    detect_record_query_presence(clauses, module_data)
  end

  defp handle_form_for_records({{:define, _line, _macro, replacement}, _form_node}, module_data) do
  ...
  defp detect_record_query_presence({:call, _, {:atom, _, :is_record}, _}, module_data),
    do: %ModuleData{module_data | has_is_record: true}

  defp detect_record_query_presence({:call, _, {:atom, _, :record_info}, [{:atom, _, :size}, _]}, module_data) do
    set_record_size_macro(module_data)
  end

  defp detect_record_query_presence({:record_index, _, _, _}, module_data), do: set_record_index_macro(module_data)

  defp detect_record_query_presence(tuple, module_data) when is_tuple(tuple),
    do: detect_record_query_presence(Tuple.to_list(tuple), module_data)

  defp detect_record_query_presence(list, module_data) when is_list(list),
    do: list |> Enum.reduce(module_data, &detect_record_query_presence/2)

  defp detect_record_query_presence(_, module_data), do: module_data

  defp set_record_size_macro(%ModuleData{record_size_macro: nil, used_func_names: used_func_names} = module_data) do
    macro_name = Utils.find_available_name("erlrecordsize", used_func_names)
    %ModuleData{module_data | record_size_macro: macro_name, used_func_names: MapSet.put(used_func_names, macro_name)}
  end

  defp set_record_size_macro(module_data), do: module_data

  defp set_record_index_macro(%ModuleData{record_index_macro: nil, used_func_names: used_func_names} = module_data) do
    macro_name = Utils.find_available_name("erlrecordindex", used_func_names)
    %ModuleData{module_data | record_index_macro: macro_name, used_func_names: MapSet.put(used_func_names, macro_name)}
  end
  ...
  defp handle_form_for_macros({{:define, _line, macro, replacement}, _form_node}, module_data) do
  ...
  defp handle_form_for_macros({{:attribute, _line, directive, name}, _form_node}, module_data)
  ...
  defp handle_form_for_macros({{:function, _line, _name, _arity, clauses}, _form_node}, module_data) do
  ...
  defp detect_func_style_call(tuple, module_data) when is_tuple(tuple),
    do: detect_func_style_call(Tuple.to_list(tuple), module_data)

  defp detect_func_style_call(list, module_data) when is_list(list),
    do: list |> Enum.reduce(module_data, &detect_func_style_call/2)
  ...
  defp update_macro_info(
    ...
    macro = %MacroData{macro | const_name: const_name}
    %ModuleData{module_data | macros: Map.put(macros, name, macro), used_attr_names: used_attr_names}
  end
  ...
  defp update_macro_info(
    ...
    macro = %MacroData{macro | func_name: func_name}
    %ModuleData{module_data | macros: Map.put(macros, name, macro), used_attr_names: used_attr_names}
  end
  ...
  defp update_macro_info(
    ...
    macro = %MacroData{macro | is_redefined: true, const_name: const_name, func_name: func_name}
  ...
  defp update_macro_info(
    ...
    macro = %MacroData{macro | is_redefined: is_redefined, const_name: const_name, const_expr: replacement_expr}
    %ModuleData{module_data | macros: Map.put(macros, name, macro), used_func_names: used_func_names}
  end
  ...
  defp update_macro_info(
    ...
    macro = %MacroData{macro | is_redefined: is_redefined, func_name: func_name}
    %ModuleData{module_data | macros: Map.put(macros, name, macro), used_func_names: used_func_names}
  end
