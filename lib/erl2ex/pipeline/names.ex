...
defmodule Erl2ex.Pipeline.Names do
  ...
  def special_attr_name?(name), do: MapSet.member?(@special_attribute_names, name)
  ...
  def callable_function_name?(name),
    do:
  ...
  def deffable_function_name?(name),
    do: callable_function_name?(name) and not MapSet.member?(@elixir_reserved_words, name)

  # Returns true if the given function name can be called without module
  # qualification.

  def local_callable_function_name?(name),
    do: deffable_function_name?(name) and not MapSet.member?(@elixir_special_forms, name)
